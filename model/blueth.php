<?php
class Blueth extends Model {
    
    protected $sessionUser;
    
    public function __construct($dsn, $user, $password) {
        parent::__construct($dsn, $user, $password);
        $this->selectSQL = 'select id, name, title, collor, prise, picture, description, material from kofta order by id desc';
        $this->countSQL = 'select count(*) from kofta';
        $this->insertSQL = 'insert into kofta (name, title, collor, prise, picture, description, material) values (?, ?, ?, ?, ?, ?, ?)';
        $this->updateformSQL = 'select name, title, collor, prise, picture, description, material from kofta where id=?';
        $this->updateSQL = 'update kofta set name=?, title=?, collor=?, prise=?, picture=?, description=?, material=? where id=?';
        $this->deleteSQL = 'delete from kofta where id=?';
        $this->aboutSQL = 'select name, title, collor, prise, picture, description, material from kofta where id=?';
