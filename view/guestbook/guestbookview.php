...
class Guestbookview{
    ...
    private function create() {
        if ($this->requestObject->isPOST && !$this->dataObject->error) {
            ...
            $url = $_SERVER['SCRIPT_NAME'] . '?model=guestbook&command=create&page=' . $this->requestObject->page;
            $caption = 'Добавить сообщение';
    ...
    private function update() {
        if ($this->requestObject->isPOST && !$this->dataObject->error) {
            ...
            $url = $_SERVER['SCRIPT_NAME'] . '?model=guestbook&command=update&page=' . $this->requestObject->page . '&id=' . $this->requestObject->id;
            $caption = 'Редактировать сообщение';
            $this->form($url, $caption);
        }        
    }
	...
	private function about() {
        if ($this->requestObject->isPOST && !$this->dataObject->error) {
            ...
            $url = $_SERVER['SCRIPT_NAME'] . '?model=guestbook&command=about&page=' . $this->requestObject->page . '&id=' . $this->requestObject->id;
            $caption = 'ABOUT';
    ...
    private function redirect($page) {
        header("Location: http://{$_SERVER['SERVER_NAME']}{$_SERVER['SCRIPT_NAME']}?model=guestbook&page=$page");
        exit();
    }
}
